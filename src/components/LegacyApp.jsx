...
import {
  applyCartDelta,
  formatFreeAddonProductIdsForCsv,
  parseFreeAddonProductIds
} from '../utils/autoFreeAddon'
import { formatAddonQtyNote, formatAddonPriceHint } from '../utils/addonDisplay'

const GUEST_CHECKOUT_LABEL = '非會員快速結帳（建立會員帳號）'

function addonItemPriceLabel(item) {
  const hint = formatAddonPriceHint(item)
  if (hint != null) return hint
  return item.subtotal === 0 ? '0' : item.price
}
...
function toAbsoluteOgUrl(origin, url) {
  ...
  return `${base}${path}`
    ...
    function App({ routeMode = 'home', routeProductId = '', standaloneAdminPage = false }) {
      ...
      const [privacySettingsRaw, setPrivacySettingsRaw] = useState(null);
      ...
      const [cartAddonFree, setCartAddonFree] = useState({});
      const cartAddonFreeRef = useRef({});
...
const [adminLogs, setAdminLogs] = useState([]);
      ...
      const privacySettings = useMemo(
        () => mergePrivacySettings(privacySettingsRaw),
        [privacySettingsRaw]
      )
...
const [publicTopSellers, setPublicTopSellers] = useState({ items: [], label: '本月' });
      ...
      const formatAdminLog = (log) => {
        ...
        } else if (log.action === 'privacy_policy_updated') {
          summary = `個資政策已更新（版本 v${d.version || '-'}）`
      ...
      useEffect(() => {
        cartAddonFreeRef.current = cartAddonFree
      }, [cartAddonFree])
      ...
      useEffect(() => {
        try {
          ...
          if (parsed && typeof parsed === 'object' && 'cart' in parsed) {
            setCart(parsed.cart || {})
            setCartAddonFree(parsed.addonFree || {})
          } else {
            setCart(parsed || {})
          }
        } catch (error) {
      ...
      useEffect(() => {
        try {
          localStorage.setItem('muzi_cart_v1', JSON.stringify({ cart, addonFree: cartAddonFree }))
        } catch (error) {
...
// =======================================================
      ...
      useEffect(() => {
        ...
        const unsubscribePrivacy = db.collection('settings').doc('privacy').onSnapshot((doc) => {
          setPrivacySettingsRaw(doc.exists ? doc.data() : null)
        })
        ...
        return () => {
          ...
          unsubscribeConfig(); unsubscribeAbout(); unsubscribePrivacy(); unsubscribeProducts(); 
      ...
      const updateCart = (id, delta) => {
        setCart((prev) => {
          const prevFree = cartAddonFreeRef.current
          const result = applyCartDelta(prev, prevFree, id, delta, products)
          if (result.blocked) {
            alert(result.message || '無法調整數量')
            return prev
          }
          setCartAddonFree(result.addonFree)
          return result.cart
        })
      };
      ...
      const cartData = useMemo(() => {
        let items = Object.entries(cart)
          .map(([id, qty]) => {
            ...
            if (product.isAddon) {
              const freeQty = Math.min(Number(cartAddonFree[id]) || 0, qty)
              item.freeQty = freeQty
              item.paidQty = Math.max(0, qty - freeQty)
            }
            return item
          })
          .filter(Boolean)
      ...
      }, [cart, cartAddonFree, products, storeConfig, deliveryMethod])

      const getDisplayQtyForProduct = (productId) => cart[productId] || 0

      const floatingCartQty = cartData.totalQty
      const floatingCartAmount = cartData.currentTotal
      ...
      const resetPrivacyConsentChecks = () => {
      ...
      const handleReorderOrder = (order) => {
        ...
        ;(order.items || []).forEach((item) => {
          ...
          if (item.isAddon && Number(item.freeQty) > 0) {
            nextAddonFree[item.id] = Number(item.freeQty)
          }
        })
        setCart(nextCart)
        setCartAddonFree(nextAddonFree)
        setShowMemberProfile(false)
        if (routeMode === 'cart') {
          setIsCartOpen(true)
        } else {
          ...
          try {
            if (isNewCustomer) {
               ...
               await db.collection('users').doc(newId).set({
                 ...
                 role: 'customer',
                 email: '',
                 electronicPrivacyConsent: 'none',
                 adminConsentNote: '無電子同意',
                 createdAt: firebase.firestore.FieldValue.serverTimestamp()
               });
      ...
      const handleCheckout = async () => {
        ...
        const checkoutPhone = String(customerInfo.phone || '').replace(/\D/g, '')
        if (!/^09\d{8}$/.test(checkoutPhone)) {
          return alert('手機格式不正確，請輸入 09 開頭共 10 碼')
        }
        ...
        if (db) {
          ...
          const orderPayload = {
            orderId, userId: finalUserId, customerInfo: { ...customerInfo, phone: checkoutPhone }, items: cartData.items,
            totals: {
...
try {
       ...
       const notifyData = {
        ...
        setCart({}); setCartAddonFree({}); setIsCartOpen(false); setOrderNote('');
                        ...
                        <tbody>
                          ${chunkItems.map(item => `
                            <tr>
                              ...
                              <td style="border: 1px solid #333; padding: 10px;">${item.name} ${item.weight ? `(${item.weight})` : ''}${item.groupSplitLabel ? `（${item.groupSplitLabel}）` : ''}${item.isAddon && formatAddonQtyNote(item) ? `<br/><span style="font-size:0.85em;color:#1d4ed8;">${formatAddonQtyNote(item)}</span>` : ''}</td>
                              <td style="border: 1px solid #333; padding: 10px;">$${addonItemPriceLabel(item)}</td>
...
const uploadTask = await storageRef.put(blob, metadata);
      ...
      const openProductDetail = (product) => { setEditingProduct({ intro: '', ingredients: '', notices: '', extraImages: [], isPromo: false, isAddon: false, providesFreeAddon: false, freeAddonProductIds: [], isNew: false, isFreeShipping: true, unit: product.unit || '', cost: 0, ...product }); setMainDisplayImg(product.image || ''); };
      const handleAddProduct = () => { setEditingProduct({ id: '', name: '', desc: '', weight: '', price: 0, cost: 0, category: adminCategoryNames[0] || '精選商品', image: '', intro: '', ingredients: '', notices: '', extraImages: [], isPromo: false, isAddon: false, providesFreeAddon: false, freeAddonProductIds: [], isNew: true, isFreeShipping: true, unit: '' }); setMainDisplayImg(''); };
      ...
      const addNewTableRow = () => {
        setTableProducts([{ id: '', name: '', desc: '', weight: '', price: 0, cost: 0, category: adminCategoryNames[0] || '精選商品', image: '', intro: '', ingredients: '', notices: '', extraImages: [], isPromo: false, isAddon: false, providesFreeAddon: false, freeAddonProductIds: [], isNew: true, isFreeShipping: true, unit: '' }, ...tableProducts]);
      };
      ...
      const handleCSVImport = async (e) => {
        ...
        reader.onload = async (event) => {
          ...
          for (let i = 1; i < lines.length; i++) {
            ...
            const providesFreeAddon = cols[11] ? cols[11].trim() === '是' : false
            const freeAddonIdsRaw = cols.length > 12 ? cols[12] : ''
            const freeAddonProductIds = providesFreeAddon
              ? parseFreeAddonProductIds(freeAddonIdsRaw)
              : []
            ...
            const prodData = {
              ...
              providesFreeAddon,
              ...(providesFreeAddon ? { freeAddonProductIds } : {}),
            };
      ...
      const handleCSVExport = () => {
        ...
        let csvContent = "data:text/csv;charset=utf-8,\uFEFF";
        csvContent += "品號,商品名稱,分類,簡介,單位,售價,成本,重量,計入免運,優惠活動,加購商品,送加購額度,指定附贈加購品號\n";
        ...
        tableProducts.forEach(item => {
          const row = [
            ...
            item.providesFreeAddon ? formatFreeAddonProductIdsForCsv(item.freeAddonProductIds) : ''
  ...
  setStoreConfig(configToSave); setShowConfigModal(false);
      ...
      const announceTextBelowItems = useMemo(
        () =>
          announcements.filter(
            (a) =>
              ...
              !isAnnounceExpired(a) &&
              !a.image &&
              (String(a.title || '').trim() || String(a.content || '').trim())
          ),
        [announcements]
      );
            ...
            <div className="max-w-5xl mx-auto bg-brand-marble w-full rounded-3xl h-[92vh] flex flex-col overflow-hidden shadow-2xl border border-stone-100">
              ...
              <div className="p-4 bg-white border-t border-stone-200">
                <div className="flex gap-4">
                  ...
                  <Link to="/cart" className="flex-[1.5] brand-btn-primary font-bold py-3.5 rounded-xl shadow-lg text-center active:scale-95 transition-transform">前往購物車</Link>
                </div>
              </div>
            </div>
      ...
      return (
        <div className="max-w-md md:max-w-4xl lg:max-w-6xl mx-auto bg-brand-marble min-h-screen relative font-sans text-stone-800 shadow-xl overflow-hidden flex flex-col">
          ...
          <div className="sticky top-0 z-20 bg-brand-marble shadow-sm border-b border-stone-100/80">
            ...
            {announceTextBelowItems.length > 0 && (
              <div className="mx-auto mt-2 mb-1 w-full max-w-md md:max-w-5xl px-4 sm:px-6 space-y-2">
                {announceTextBelowItems.map((ann) => (
                  <button
                    key={ann.id}
                    type="button"
                    onClick={() => {
                    ...
                    className="w-full text-left rounded-xl border border-[#D4C4B0] bg-gradient-to-r from-[#F5F0E8] to-[#EBE3D6] px-4 py-3 shadow-sm hover:border-[#8B7355] hover:shadow-md transition-all active:scale-[0.99]"
                  >
                    {ann.title ? (
                      <p className="text-sm font-black text-[#5C4A3A] flex items-center gap-2">
                        <Megaphone size={16} className="text-[#8B7355] shrink-0" />
                        {ann.title}
                      </p>
                    ) : null}
                    {ann.content ? (
          ...
          <main className="flex-1 overflow-y-auto pb-24 px-4">
           {/* --- 首頁客戶端熱銷排行榜 (公用看板版) --- */}
            {(!isAdminMode || adminOrderingFor) && publicTopSellers.items && publicTopSellers.items.length >= 3 && activeCategory === '全部' && (
              <div className="mt-6 mb-8 bg-brand-marble p-5 rounded-2xl shadow-lg border border-stone-100 relative overflow-hidden">
              ...
              <footer className="mt-10 mb-2 pt-8 border-t border-stone-200">
                <nav className="flex flex-wrap justify-center gap-x-5 gap-y-2 text-sm text-stone-600">
                  ...
                  {currentUser ? (
                    ...
                    <span className="text-stone-400 cursor-not-allowed" title="請先登入會員">
                      會員中心
                    </span>
                  )}
                </nav>
  ...
  買此商品送0元加購額度
                        ...
                        {editingProduct.providesFreeAddon && (
                          <div className="rounded-xl border border-blue-200 bg-blue-50/80 p-3 space-y-2 mb-2">
                            <p className="text-xs font-bold text-blue-800">指定附贈加購品（可選）</p>
                            <p className="text-[10px] text-blue-700 leading-relaxed">
                              不勾選任何項目＝自動附贈「全部」加購商品；有勾選則只送勾選的品項。
                            </p>
                            {addonProducts.length === 0 ? (
                              <p className="text-[10px] text-stone-500">尚無加購商品，請先將品項設為「加購」。</p>
                            ) : (
                              <div className="flex flex-wrap gap-2">
                                {addonProducts.map((addon) => {
                                  const aid = String(addon.id || '').toUpperCase()
                                  const selected = (editingProduct.freeAddonProductIds || []).some(
                                    (x) => String(x).toUpperCase() === aid
                                  )
                                  return (
                                    <label
                                      key={addon.id}
                                      className={`flex items-center gap-1.5 text-[10px] px-2 py-1 rounded-md cursor-pointer border ${
                                        selected
                                    ...
                                    >
                                      <input
                                        ...
                                        checked={selected}
                                        onChange={() => {
                                          const cur = [...(editingProduct.freeAddonProductIds || [])]
                                          const idx = cur.findIndex((x) => String(x).toUpperCase() === aid)
                                          if (idx >= 0) cur.splice(idx, 1)
                                          else cur.push(aid)
                                          setEditingProduct({
                                            ...editingProduct,
                                            freeAddonProductIds: cur
                                          })
                                        }}
          ...
          {floatingCartQty > 0 &&
            ...
            <div className="fixed bottom-0 left-0 right-0 max-w-md md:max-w-4xl lg:max-w-6xl mx-auto p-4 bg-gradient-to-t from-white via-white to-transparent pointer-events-none z-[34]">
              <Link to="/cart" className="w-full bg-stone-800 text-white rounded-2xl p-4 flex items-center justify-between shadow-xl pointer-events-auto active:scale-95 transition-transform">
                <div className="flex items-center gap-3"><div className="relative"><ShoppingCart size={24} /><span className="absolute -top-2 -right-2 bg-red-600 text-white text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center">{floatingCartQty}</span></div><span className="font-medium">查看購物車</span></div>
                <div className="text-lg font-bold">${floatingCartAmount}</div>
              </Link>
            </div>
          )}
          </>
                              ...
                              <div className="text-stone-600 text-xs space-y-1.5 mb-4 bg-stone-50 p-3 rounded-lg">
                                {order.items.map((item, i) => (
                                  <div key={i} className="flex justify-between items-center">
                                    <span>{item.name} {item.weight && <span className="text-[10px] text-stone-500 font-normal ml-1">({item.weight})</span>} {item.isAddon && <span className="text-[10px] bg-purple-100 text-purple-600 px-1 rounded">加購</span>} {item.isGift && <span className="text-[10px] bg-emerald-100 text-emerald-600 px-1 rounded font-bold">滿件贈品</span>} <span className="text-stone-400 text-[10px]">(${addonItemPriceLabel(item)})</span>{item.isAddon && formatAddonQtyNote(item) ? <span className="block text-[10px] text-blue-700 font-bold">{formatAddonQtyNote(item)}</span> : null}</span>
...
// 🌟 新增：讓排行榜直接讀取小白板上算好的商品銷量
  ...
  <button onClick={handleCloudSearch} className="bg-stone-800 text-white text-xs font-bold px-3 py-1.5 rounded hover:bg-stone-700 transition-colors shadow-sm shrink-0 active:scale-95">搜尋</button>
          ...
          {showAdminCustomers && isAdminMode && (
            <AdminCustomersModal
              ...
              navigate={navigate}
              statusMap={STATUS_MAP}
              onDownloadPrivacyConsentPdf={handleDownloadCustomerConsentPdf}
            />
          )}

          {showAdminPrivacyModal && isAdminMode && (
            <AdminPrivacyModal
              onClose={() => setShowAdminPrivacyModal(false)}
                  ...
                  <table className="w-full text-left border-collapse min-w-[1200px]">
                    <thead className="bg-stone-100 sticky top-0 z-10 shadow-sm">
                      <tr>
                        ...
                        <th className="p-2 border-b border-stone-200 text-stone-600 font-bold text-sm w-36">指定附贈加購品</th>
                      </tr>
                    </thead>
                    <tbody>
  ...
  <input type="checkbox" checked={item.providesFreeAddon || false} onChange={e => handleTableFieldChange(index, 'providesFreeAddon', e.target.checked)} className="w-4 h-4 accent-blue-500 cursor-pointer" />
                          ...
                          <td className="p-2 border-b border-stone-200">
                            {item.providesFreeAddon ? (
                              <input
                                type="text"
                                value={formatFreeAddonProductIdsForCsv(item.freeAddonProductIds)}
                                onChange={(e) =>
                                  handleTableFieldChange(
                                    index,
                                    'freeAddonProductIds',
                                    parseFreeAddonProductIds(e.target.value)
                                  )
                                }
                                placeholder="空白=全部; D02;D03"
                                className="w-full bg-transparent border border-transparent hover:border-stone-300 focus:border-[#8B7355] focus:bg-white rounded px-1 py-0.5 text-[10px] outline-none"
                              />
                            ) : (
          ...
          {showLoginModal && (
            <div className="fixed inset-0 z-50 flex justify-end sm:justify-center items-end sm:items-center bg-black/50 backdrop-blur-sm p-4">
              <div
                className={`bg-brand-marble rounded-3xl shadow-2xl w-full animate-in zoom-in-95 duration-200 relative border border-stone-100 flex flex-col max-h-[92vh] ${
                  isRegistering && loginMode === 'customer' ? 'max-w-lg' : 'max-w-md'
                }`}
              >
                ...
                <div className="shrink-0 px-6 md:px-8 pt-6 pb-2 pr-12">
                <h3 className="text-xl font-bold text-stone-800 flex items-center gap-2 justify-center">
                ...
                <div className="flex-1 min-h-0 overflow-y-auto px-6 md:px-8 overscroll-contain">
                {forgotPasswordPanelOpen && !isRegistering ? (
                ...
                <div className="shrink-0 px-6 md:px-8 pb-6 pt-3 border-t border-stone-100 bg-brand-marble rounded-b-3xl">
                ...
                <div className="mt-4 pt-3 flex justify-between items-center text-xs text-stone-500">
                  {loginMode === 'customer' ? (
                    <>
                      <button
                        type="button"
